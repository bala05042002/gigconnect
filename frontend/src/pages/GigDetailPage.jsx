...
const GigDetailPage = () => {
    ...
    return (
        <div className="relative min-h-screen bg-gray-900 text-white p-4 md:p-8 overflow-hidden">
            ...
            {isGigOwner && (
                <div className="bg-black/70 rounded-lg shadow-md p-6 mt-8 max-w-4xl mx-auto">
                    <h2 className="text-xl sm:text-2xl font-bold mb-4">Applications ({bids.length})</h2>
                    {bids.length ? bids.map(bid => (
                        <div key={bid._id} className="bg-gray-800 p-4 rounded-lg border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                                <h3><Link to={`/profiles/${bid.user?._id}`} className="hover:underline">{bid.user?.name}</Link></h3>
                                <span className="font-bold text-green-600">${bid?.price}</span>
                            </div>
                            <p>{bid?.proposal}</p>
                            {isGigOpen && <button onClick={() => handleAcceptBid(bid._id)} className="mt-2 px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700">Accept</button>}
                        </div>
