    ...
    useEffect(() => {
        const fetchMessages = async () => {
            ...
            try {
                ...
                const mockMessages = [
                    { sender: receiverId, content: "Hey, are you free to chat about the gig?", timestamp: Date.now() - 60000 },
                    { sender: user._id, content: "Yes, I am. How can I help?", timestamp: Date.now() - 50000 },
                    { sender: receiverId, content: "I have a few questions about your proposal.", timestamp: Date.now() - 40000 }
                ];
        ...
        <div className="fixed inset-0 bg-gray-100 flex flex-col z-50">
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-4">
                {messages.map((msg, index) => (
                    ...
                    >
                        <div className={`px-4 py-2 rounded-lg max-w-[75%] shadow-md ${msg.sender === user._id ? 'bg-indigo-600 text-white' : 'bg-white text-gray-800'}`}>
                            {msg.content}
                        </div>
                    </div>
                ))}
